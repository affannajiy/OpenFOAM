...
# Minimal valid dictionaries written into a NEW case so OpenFOAM tools can run
# immediately. controlDict is required by every OF executable; fvSchemes and
# fvSolution are required to exist even though meshing never reads them.
...
class LandingWidget(QWidget):
    ...
    def _build_hero(self, content: QVBoxLayout):
        """Build the welcome strip: title + short how-it-works text on the
        left, a small environment card (working dir, OpenFOAM version,
        ParaView detection) on the right."""
        frame = QFrame()
    ...
    def _build_columns(self, content: QVBoxLayout):
        """Lay out the two main cards side by side: 01 Project (new/open) and
        02 Utility (which tool to enter)."""
        cols = QHBoxLayout()
        cols.setSpacing(20)
    ...
    def _build_project_card(self, body: QVBoxLayout):
        """Fill the Project card: a New/Open segmented switch on top and a
        QStackedWidget below it holding the two mode forms."""
        # Segmented control
        seg = QFrame()
    ...
    def _build_new_project_form(self, vbox: QVBoxLayout):
        """Build the New-project form: name (live-sanitized), location
        (validated as WSL-reachable), template picker (Empty / From STL with an
        inline multi-file picker), and a folder-tree preview of what will be
        created."""
    ...
    def _build_open_existing_form(self, vbox: QVBoxLayout):
        """Build the Open-existing form: a browse button, the selected-path
        label, and the scrollable recent-projects list."""
    ...
    def _make_recent_row(self, entry: dict) -> QFrame:
        """Build one clickable row for the recents list: project name + path on
        the left, a Meshed/Draft status pill, and an × remove button."""
        path   = entry.get("path", "")
    ...
    def _on_recent_row_click(self, path: str, frame: QFrame):
        """Select a recent project: un-highlight the previous row, highlight
        this one, and set it as the path the Open button will use."""
    ...
    def _build_utility_card(self, body: QVBoxLayout):
        """Fill the Utility card: one selectable sub-card per tool (Background
        Mesh, Snappy Hex Mesh) plus a small environment checklist below."""
    ...
    def _make_util_selector(self, uid: int, title: str, chip: str, desc: str) -> QFrame:
        """Build one utility sub-card: title + typical-duration chip +
        description. Single click selects it, double click opens it."""
        card = QFrame()
    ...
    def _style_util_card(self, card: QFrame, selected: bool):
        """Paint a utility sub-card: red outline + light-red fill when
        selected, plain bordered card otherwise."""
    ...
    def _on_util_double_click(self, uid: int):
        """Double-click shortcut: select the utility and open it in one go."""
    ...
    def _set_mode(self, mode: str):
        """Switch between the "new" and "open" project forms."""
        self._mode = mode
    ...
    def _refresh_seg_control(self):
        """Restyle the New/Open segmented switch — the active side looks like a
        raised white button, the inactive side is flat grey text."""
        active_ss = (
    ...
    def _update_preview(self):
        """Redraw the WILL CREATE folder-tree preview from the current name and
        location fields."""
    ...
    def _on_continue(self):
        """Open button: in "new" mode create the case skeleton on disk (folders,
        stub dicts, optional STL copies); in "open" mode validate the chosen
