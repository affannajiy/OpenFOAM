...
class LogDrawer(QWidget):
    ...
    def _build(self):
        """Assemble the drawer top-to-bottom: resize grip, header strip (dot,
        OUTPUT LOG label, step label, line counter, Copy/Clear/chevron
        buttons), then the read-only text area shown only when expanded."""
    ...
    def _clear(self):
        """Wipe all log text and reset the line counter / header buttons."""
        self._text.clear()
        self._line_count = 0
    ...
    def _copy_log(self):
        """Copy the entire log to the clipboard and flash '✓ Copied' on the
        button for 2 seconds as feedback."""
    ...
    def _reset_copy_btn(self):
        """Restore the Copy button after the '✓ Copied' flash."""
        self._copy_btn.setText("Copy")
        self._copy_btn.setEnabled(True)

    def _do_blink(self):
        """Timer tick while a job runs: alternate the dot between bright and
        dark amber to make it visibly pulse."""
