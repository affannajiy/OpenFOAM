...
Checks:
  - Python version (3.8+)
  - jinja2         (required — all runs of setup_snappy.py)
  - trimesh        (optional — AUTO_ auto-refinement, flip_normals.py,
                               find_interior_point.py)
  - numpy          (optional — required alongside trimesh)
"""
...
def main():
    ...
    print()
    print("Optional packages (AUTO_ auto-refinement, flip_normals.py,")
    print("                   find_interior_point.py):")
    trimesh_ok = check_package("trimesh",  min_version="4.0",  required=False)
    numpy_ok   = check_package("numpy",    min_version="1.24", required=False)

    print()
    print("─" * 45)

    hard_fail = not py_ok or not jinja_ok
    soft_warn = not trimesh_ok or not numpy_ok
    ...
    if hard_fail:
        ...
        print(f"{YELLOW}Ready for basic use.{RESET} Optional packages missing —")
        print(f"  AUTO_, flip_normals.py and find_interior_point.py will not work.")
        print(f"  Install optional deps:  pip install trimesh numpy")
    else:
