...
def validate_config(config):
    ...
    # numCores is mandatory in snappy_inputs.json (never in defaults.json)
    if "numCores" not in settings:
        sys.exit(
            "Error: 'settings.numCores' is required in snappy_inputs.json.\n"
            "       Set the number of parallel subdomains, e.g. \"numCores\": 4"
        )
    if not isinstance(settings["numCores"], int) or settings["numCores"] < 1:
        sys.exit(
            f"Error: 'settings.numCores' must be a positive integer, got {settings['numCores']!r}.\n"
            "       Example: \"numCores\": 4"
        )

    # castellatedMeshControls: require exactly one of locationInMesh / locationsInMesh
    castellated = config.get("castellatedMeshControls", {})
    has_single = "locationInMesh" in castellated
    has_multi  = "locationsInMesh" in castellated

    if has_single and has_multi:
        sys.exit(
            "Error: 'castellatedMeshControls' must have either 'locationInMesh' or "
            "'locationsInMesh', not both."
        )
    if not has_single and not has_multi:
        sys.exit(
            "Error: 'castellatedMeshControls' must contain 'locationInMesh' (single point) or "
            "'locationsInMesh' (multiple named points).\n"
            "       Single:   \"locationInMesh\": [x, y, z]\n"
            "       Multiple: \"locationsInMesh\": [{\"point\": [x, y, z], \"name\": \"zone\"}, ...]"
        )
    ...
    if has_multi:
        locs = castellated["locationsInMesh"]
        if not isinstance(locs, list) or len(locs) == 0:
            sys.exit(
                "Error: 'castellatedMeshControls.locationsInMesh' must be a non-empty list.\n"
                "       Each entry: {\"point\": [x, y, z], \"name\": \"zoneName\"}"
            )
        for i, entry in enumerate(locs):
            if not isinstance(entry, dict):
                sys.exit(
                    f"Error: 'castellatedMeshControls.locationsInMesh[{i}]' must be a dict "
                    f"with 'point' and 'name'."
                )
            if "point" not in entry or "name" not in entry:
                sys.exit(
                    f"Error: 'castellatedMeshControls.locationsInMesh[{i}]' must have "
                    f"both 'point' and 'name'."
                )
            pt = entry["point"]
            if not isinstance(pt, list) or len(pt) != 3 or not all(isinstance(v, (int, float)) for v in pt):
                sys.exit(
                    f"Error: 'castellatedMeshControls.locationsInMesh[{i}].point' must be "
                    f"a list of 3 numbers. Got: {pt}"
                )
            nm = entry["name"]
            if not isinstance(nm, str) or not nm.strip():
                sys.exit(
                    f"Error: 'castellatedMeshControls.locationsInMesh[{i}].name' must be "
                    f"a non-empty string. Got: {nm!r}"
                )
...
def validate_surface_handling_section(surf_section, geometry_map, extract_from_names=False):
    ...
    forbidden_in_defaults = {"faceZoneName", "cellZoneName", "regions", "namedRegions"}
    surf_auto_keys = set()
    ...
    for name, entry in surfaces.items():
        ...
        if name == "__defaults__":
            if "autoRefine" in entry:
                sys.exit("Error: 'autoRefine' is not allowed in surfaceHandling.surfaces.__defaults__.")
        ...
        if name not in selected_parts:
            is_auto_entry = (
                name in geometry_map and (
                    geometry_map[name]['encoded'].get('is_auto', False) or
                    entry.get('autoRefine') is True
                )
            )
        ...
        if entry.get("autoRefine") is True and "refinementLevels" in entry:
            sys.exit(
                f"Error: surfaceHandling.surfaces['{name}'] has both 'autoRefine: true' and "
                f"'refinementLevels' — these are mutually exclusive."
            )

        if entry.get("autoRefine") is True:
            if geometry_map[name]['geom'].get('is_standard_shape', False):
                sys.exit(
                    f"Error: 'autoRefine: true' is not supported for standard shapes ('{name}').\n"
                    f"       autoRefine requires a geometry file. Use explicit refinementLevels instead."
                )
            surf_auto_keys.add(name)
        ...
        if "namedRegions" in entry:
            if not isinstance(entry["namedRegions"], list):
                sys.exit(f"Error: surfaceHandling.surfaces['{name}'].namedRegions must be a list")
            for i, rname in enumerate(entry["namedRegions"]):
                if not isinstance(rname, str) or not rname.strip():
                    sys.exit(
                        f"Error: surfaceHandling.surfaces['{name}'].namedRegions[{i}] must be a non-empty string"
                    )

    return surf_auto_keys
...
def validate_volume_refinement_section(vol_section, geometry_map, extract_from_names=False):
    ...
    for name, entry in regions.items():
        ...
        if name == "__defaults__":
            if "autoRefine" in entry:
                sys.exit("Error: 'autoRefine' is not allowed in volumeRefinement.regions.__defaults__.")
            continue
        ...
        if entry.get("autoRefine") is True and "level" in entry:
            sys.exit(
                f"Error: volumeRefinement.regions['{name}'] has both 'autoRefine: true' and "
                f"'level' — these are mutually exclusive."
            )
        ...
        if entry.get("autoRefine") is True:
            if geometry_map[name]['geom'].get('is_standard_shape', False):
                sys.exit(
                    f"Error: 'autoRefine: true' is not supported for standard shapes ('{name}').\n"
                    f"       autoRefine requires a geometry file. Use explicit level instead."
                )
            vol_auto_keys.add(name)
    ...
    return vol_auto_keys


def compact_numeric_arrays(json_str):
    """Collapse numeric-only JSON arrays onto a single line: [1, 2] instead of multi-line."""
    return re.sub(
        r'\[\s*([\d\s.,eE+\-]+?)\s*\]',
...
def write_back_auto_levels(config_file, surf_auto_keys, vol_auto_keys, auto_levels_map):
    """
    Write auto-derived refinement levels back to snappy_inputs.json.

    For surface entries with autoRefine: true:
      - Writes concrete refinementLevels: [surface_min, surface_max]
      - Removes the autoRefine key
      - Clears the regions sub-dict (parent auto = clean slate)

    For volume entries with autoRefine: true:
      - Writes concrete level: volume_level
      - Removes the autoRefine key
    """
    try:
        with open(config_file, 'r') as f:
        ...
        sys.exit(f"Error: Could not read {config_file} for auto-level write-back: {e}")

    surfaces = case_config.get("surfaceHandling", {}).get("surfaces", {})
    for key in surf_auto_keys:
        if key in surfaces and isinstance(surfaces[key], dict):
            entry = surfaces[key]
            if key in auto_levels_map:
                auto = auto_levels_map[key]
                entry["refinementLevels"] = [auto["surface_min"], auto["surface_max"]]
            entry.pop("autoRefine", None)
            entry.pop("regions", None)

    vol_regions = case_config.get("volumeRefinement", {}).get("regions", {})
    for key in vol_auto_keys:
        if key in vol_regions and isinstance(vol_regions[key], dict):
            entry = vol_regions[key]
            if key in auto_levels_map:
                auto = auto_levels_map[key]
                entry["level"] = auto["volume_level"]
            entry.pop("autoRefine", None)

    try:
        with open(config_file, 'w') as f:
            f.write(compact_numeric_arrays(json.dumps(case_config, indent=4)))
        print(f"  Auto-derived levels written back to {config_file}")
    except OSError as e:
        sys.exit(f"Error: Could not write back to {config_file}: {e}")
...
def process_geometry(config, extract_from_names, convention):
    ...
    if "files" in geometry:
        ...
        for filename in filenames:
            ...
            if file_stem.startswith("AUTO_"):
                ...
                parsed = empty_encoded_result(file_stem)
                geom["name"] = file_stem
    ...
    for idx, shape in enumerate(geometry.get("standardShapes", [])):
        ...
        if extract_from_names:
            ...
            parsed = empty_encoded_result(raw_key)
...
def _build_candidate_set(selected_parts, geometry_map, extract_from_names, auto_keys, enc_field):
    """
    Build the ordered list of geometry keys to process.
    - Always includes explicit selectedParts
    - AUTO_-encoded entries with a non-None enc_field are always auto-selected
    - Regular encoded entries with a non-None enc_field are added when extract_from_names=True
    - Keys in auto_keys (JSON autoRefine=true) are always added
    """
    ...
    for raw_key, entry in geometry_map.items():
        ...
        if raw_key in candidate_set:
            continue
        if enc.get('is_auto') and enc[enc_field] is not None:
            candidate_set.append(raw_key)
        elif extract_from_names and enc['has_encoding'] and enc[enc_field] is not None:
            candidate_set.append(raw_key)
        elif auto_keys and raw_key in auto_keys:
            candidate_set.append(raw_key)
    return candidate_set


def resolve_surface_handling(config, geometry_map, extract_from_names, auto_levels_map=None, surf_auto_keys=None):
    """
    ...
    # Build candidate set
    candidate_set = _build_candidate_set(selected_parts, geometry_map, extract_from_names, surf_auto_keys, 'surf_type')
    ...
    for part_name in candidate_set:
        ...
        # Non-level structural fields always applied from explicit entry
        for key in ('type', 'faceType', 'faceZoneName', 'cellZoneInside', 'cellZoneName', 'namedRegions'):
            if key in explicit:
                resolved[key] = explicit[key]

        # autoRefine: true → levels from auto_levels_map; regions wiped (Gap 2 decision)
        if explicit.get('autoRefine') is True:
            if auto_levels_map and part_name in auto_levels_map:
                auto = auto_levels_map[part_name]
                resolved['refinementLevels'] = [auto['surface_min'], auto['surface_max']]
            resolved['regions'] = None
            resolved['namedRegions'] = None
        else:
            # Explicit refinementLevels and regions override
            for key in ('refinementLevels', 'regions'):
                if key in explicit:
                    resolved[key] = explicit[key]
...
def resolve_volume_refinement(config, geometry_map, extract_from_names, auto_levels_map=None, vol_auto_keys=None):
    """
    ...
    # Build candidate set
    candidate_set = _build_candidate_set(selected_parts, geometry_map, extract_from_names, vol_auto_keys, 'vol_mode')
...
def _propagate_named_regions_to_geometry(geometry, surface_refinements):
    """
    Back-propagate named region information from resolved surface refinements to
    the geometry list, so that multi-region STL files get a 'regions {}' block
    in the snappyHexMeshDict geometry section.

    Priority (first match wins):
      1. 'namedRegions' explicitly set on the resolved surface entry.
      2. 'regions' dict present on the resolved surface — region names are
         derived from its keys (used when per-region refinement levels are set).
      3. Neither present → geom.regions remains unset (no regions block written).
    """
    surf_by_name = {surf['name']: surf for surf in surface_refinements}
    for geom in geometry:
        if geom.get('is_standard_shape'):
            continue
        surf = surf_by_name.get(geom['name'])
        if surf is None:
            continue
        named = surf.get('namedRegions')
        if named:
            geom['regions'] = [{'originalName': r, 'renamedAs': r} for r in named]
        elif surf.get('regions'):
            geom['regions'] = [{'originalName': k, 'renamedAs': k} for k in surf['regions']]
...
def _get_jinja_env(template_dir):
    """Return a Jinja2 Environment loading templates from template_dir."""
...
def render_template(config, geometry, surface_refinements, volume_refinements, template_dir, openfoam_version="2506"):
    ...
    try:
        cmc = config["castellatedMeshControls"]
        location_in_mesh   = cmc.get("locationInMesh")
        locations_in_mesh  = cmc.get("locationsInMesh")
        rendered = template.render(
            ...
            locations_in_mesh=locations_in_mesh,
...
def _write_dict_file(rendered_content, output_dir, filename):
    """Write rendered content to output_dir/filename. Exits on I/O error."""
...
def write_snappyhexmeshdict(rendered_content, output_dir="system"):
    ...
    return _write_dict_file(rendered_content, output_dir, "snappyHexMeshDict")
...
def write_blockmeshdict(rendered_content, output_dir="system"):
    ...
    return _write_dict_file(rendered_content, output_dir, "blockMeshDict")


def render_decompose_par_template(num_cores, openfoam_version, template_dir):
    """Render decomposeParDict template with the requested number of subdomains."""
    env = _get_jinja_env(template_dir)
    try:
        template = env.get_template("decomposeParDict.template")
    except TemplateNotFound:
        sys.exit(f"Error: decomposeParDict.template not found in {template_dir}")

    try:
        rendered = template.render(openfoamVersion=openfoam_version, numCores=num_cores)
    except Exception as e:
        sys.exit(f"Error: Failed to render decomposeParDict template: {e}")

    return rendered


def write_decompose_par_dict(rendered_content, output_dir="system"):
    """Write rendered decomposeParDict to output directory."""
    return _write_dict_file(rendered_content, output_dir, "decomposeParDict")


def render_control_dict_template(openfoam_version, template_dir):
    """Render controlDict template."""
    env = _get_jinja_env(template_dir)
    try:
        template = env.get_template("controlDict.template")
    except TemplateNotFound:
        sys.exit(f"Error: controlDict.template not found in {template_dir}")

    try:
        rendered = template.render(openfoamVersion=openfoam_version)
    except Exception as e:
        sys.exit(f"Error: Failed to render controlDict template: {e}")

    return rendered


def write_control_dict(rendered_content, output_dir="system"):
    """Write rendered controlDict to output directory."""
    return _write_dict_file(rendered_content, output_dir, "controlDict")


def render_fv_schemes_template(openfoam_version, template_dir):
    """Render fvSchemes template."""
    env = _get_jinja_env(template_dir)
    try:
        template = env.get_template("fvSchemes.template")
    except TemplateNotFound:
        sys.exit(f"Error: fvSchemes.template not found in {template_dir}")

    try:
        rendered = template.render(openfoamVersion=openfoam_version)
    except Exception as e:
        sys.exit(f"Error: Failed to render fvSchemes template: {e}")

    return rendered


def write_fv_schemes(rendered_content, output_dir="system"):
    """Write rendered fvSchemes to output directory."""
    return _write_dict_file(rendered_content, output_dir, "fvSchemes")


def render_fv_solution_template(openfoam_version, template_dir):
    """Render fvSolution template."""
    env = _get_jinja_env(template_dir)
    try:
        template = env.get_template("fvSolution.template")
    except TemplateNotFound:
        sys.exit(f"Error: fvSolution.template not found in {template_dir}")

    try:
        rendered = template.render(openfoamVersion=openfoam_version)
    except Exception as e:
        sys.exit(f"Error: Failed to render fvSolution template: {e}")

    return rendered


def write_fv_solution(rendered_content, output_dir="system"):
    """Write rendered fvSolution to output directory."""
    return _write_dict_file(rendered_content, output_dir, "fvSolution")
...
def _apply_location_offset(config, offset=1e-6):
    """
    Apply a tiny positive offset to every point in locationInMesh or
    locationsInMesh to prevent the point(s) from coinciding with a
    background-mesh cell face.

    Modifies *config* in-place.  Must be called after validate_config()
    so the structure is known to be well-formed.  The offset is applied
    only to the in-memory dict — the original snappy_inputs.json is
    never modified.

    Parameters
    ----------
    config : dict
        The merged configuration dict (already validated).
    offset : float
        Offset magnitude in geometry units (default 1e-6 mm).
    """
    cmc = config.get("castellatedMeshControls", {})
    n_points = 0

    if "locationInMesh" in cmc:
        pt = cmc["locationInMesh"]
        cmc["locationInMesh"] = [v + offset for v in pt]
        n_points = 1

    if "locationsInMesh" in cmc:
        for entry in cmc["locationsInMesh"]:
            entry["point"] = [v + offset for v in entry["point"]]
            n_points += 1

    if n_points:
        print(f"  Offset of {offset} mm applied to {n_points} mesh location point(s).")
...
def main():
    ...
    # Offset location points so they don't coincide with cell faces
    _apply_location_offset(config)

    extract_from_names = config["settings"].get("extractRefinementFromNames", False)
    convention = config["encodingConvention"]
    of_version_str = OPENFOAM_VERSION
    ...
    if "surfaceHandling" in config:
        surf_auto_keys = validate_surface_handling_section(config["surfaceHandling"], geometry_map, extract_from_names)

    if "volumeRefinement" in config:
        vol_auto_keys = validate_volume_refinement_section(config["volumeRefinement"], geometry_map, extract_from_names)

    # Auto-refinement: detect AUTO_-encoded filenames AND JSON autoRefine: true entries
    has_filename_auto = any(
        entry['encoded'].get('is_auto', False) for entry in geometry_map.values()
    )
    json_auto_keys = surf_auto_keys | vol_auto_keys

    auto_levels_map = {}
    if has_filename_auto or json_auto_keys:
        print("Computing auto-refinement levels...")
        auto_params = validate_auto_refinement_params(config)
        auto_levels_map = compute_auto_levels_for_geometry(
            geometry_map, bm_config, auto_params, json_auto_keys=json_auto_keys
        )
        print(f"  Auto-refinement complete: {len(auto_levels_map)} geometry file(s) processed.")
        if json_auto_keys:
            write_back_auto_levels("snappy_inputs.json", surf_auto_keys, vol_auto_keys, auto_levels_map)

    print("Resolving surface handling...")
    surface_refinements = resolve_surface_handling(
        config, geometry_map, extract_from_names, auto_levels_map, surf_auto_keys
    )

    # Propagate named region info from surface refinements to geometry entries
    # so that multi-region STL files get a 'regions {}' block in the geometry section.
    _propagate_named_regions_to_geometry(geometry, surface_refinements)

    print("Resolving volume refinement...")
    volume_refinements = resolve_volume_refinement(
        config, geometry_map, extract_from_names, auto_levels_map, vol_auto_keys
    )
    ...
    print("Rendering decomposeParDict template...")
    num_cores = config["settings"]["numCores"]
    if num_cores > 1:
        dp_rendered = render_decompose_par_template(num_cores, of_version_str, template_dir)

        print("Writing decomposeParDict...")
        dp_file = write_decompose_par_dict(dp_rendered)

        print(f"✅ Successfully generated {dp_file}")
        print(f"   numberOfSubdomains: {num_cores}")
    else:
        print("ℹ️  numCores = 1 — skipping decomposeParDict (serial run, not needed)")

    print("Rendering controlDict template...")
    cd_rendered = render_control_dict_template(of_version_str, template_dir)

    print("Writing controlDict...")
    cd_file = write_control_dict(cd_rendered)

    print(f"✅ Successfully generated {cd_file}")

    print("Rendering fvSchemes template...")
    fvs_rendered = render_fv_schemes_template(of_version_str, template_dir)

    print("Writing fvSchemes...")
    fvs_file = write_fv_schemes(fvs_rendered)

    print(f"✅ Successfully generated {fvs_file}")

    print("Rendering fvSolution template...")
    fvsol_rendered = render_fv_solution_template(of_version_str, template_dir)

    print("Writing fvSolution...")
    fvsol_file = write_fv_solution(fvsol_rendered)

    print(f"✅ Successfully generated {fvsol_file}")
