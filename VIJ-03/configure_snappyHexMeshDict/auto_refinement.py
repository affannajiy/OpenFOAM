...
def validate_auto_refinement_params(config):
    ...
    All fields are optional — defaults live in defaults.json:
        surfaceResolutionCells  — positive number, typical range 3–10, default 5.0
    ...
    for key in ("surfaceResolutionCells", "featureResolutionCells",
                "volumeResolutionCells", "featureAngle", "noiseRatio", "gapMultiplier"):
        if key in params:
            v = params[key]
...
def compute_auto_levels_for_geometry(geometry_map, bm_config, auto_params, json_auto_keys=None):
    """
    ...
    _json_auto_keys = set(json_auto_keys) if json_auto_keys else set()

    for raw_key, entry in geometry_map.items():
        enc = entry['encoded']
        is_filename_auto = enc.get('is_auto', False)
        is_json_auto = raw_key in _json_auto_keys

        if not is_filename_auto and not is_json_auto:
            continue
        if entry['geom'].get('is_standard_shape', False):
            continue  # standard shapes don't have files; rejected upstream; skip defensively
