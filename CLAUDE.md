...
- **`ui_shared.py`** — colour tokens, style constants, `PlusMinusSpinBox` (QSpinBox replacement), `ChevronComboBox` (QComboBox replacement — real `▼` QLabel child instead of a QSS-drawn arrow; see Qt5 stylesheet rules below), `build_card`, `positive_float`, `get_stl_zone_names`, `find_paraview_exe`, `to_wsl_path` (Windows→`/mnt/`, applied to `QFileDialog` results), `run_of_command` (streaming), `run_foam_cmd` (blocking). Also `STYLE_TOOLTIP` — single tooltip look (white bg, black font, red rounded border); see tooltip rules. **`OF_ERROR_MAP` + `scan_log_for_fix(text)`** — scans raw OpenFOAM log for known signatures (locationInMesh, `selected 0 cells`, missing blockMeshDict, jinja2, negative volume, `FOAM FATAL ERROR`, …) → jargon-free fix or `None`. **`MessageBanner(QWidget)`** — reusable result strip above the log in both tabs: `show_error(msg)` (red ✕), `show_success(msg, action_label=None, action_cb=None)` (green ✓ + optional button), `hide_msg()`. Needs `WA_StyledBackground` + appends `STYLE_TOOLTIP`.
